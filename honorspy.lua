...
local function StartInspecting(unitID)
	...
	player.race = UnitRace(player.unitID); -- same
end
...
function HonorSpy:Report(playerOfInterest)
	...
	--Find playerOfInterest's faction
	local HSPOIClass = HonorSpy.db.realm.hs.currentStandings[playerOfInterest].class
	HSPOIClass = string.lower(HSPOIClass)
	HSPOIClass = string.gsub(HSPOIClass,"^%l", string.upper)

	local HSfaction = ""
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["High Elf"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["High Elf"][HSPOIClass][playerOfInterest] then HSfaction = "A" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Human"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Human"][HSPOIClass][playerOfInterest] then HSfaction = "A" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Dwarf"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Dwarf"][HSPOIClass][playerOfInterest] then HSfaction = "A" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Gnome"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Gnome"][HSPOIClass][playerOfInterest] then HSfaction = "A" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Night Elf"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Night Elf"][HSPOIClass][playerOfInterest] then HSfaction = "A" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Orc"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Orc"][HSPOIClass][playerOfInterest] then HSfaction = "H" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Troll"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Troll"][HSPOIClass][playerOfInterest] then HSfaction = "H" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Tauren"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Tauren"][HSPOIClass][playerOfInterest] then HSfaction = "H" end end
	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Undead"][HSPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Undead"][HSPOIClass][playerOfInterest] then HSfaction = "H" end end
	...
	local standingPOI = t[i][1]
		...
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Dwarf"][standingPOIClass] then	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Dwarf"][standingPOIClass][standingPOI] then tHSfaction = "A" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Gnome"][standingPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Gnome"][standingPOIClass][standingPOI] then tHSfaction = "A" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Night Elf"][standingPOIClass] then	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Night Elf"][standingPOIClass][standingPOI] then tHSfaction = "A" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Orc"][standingPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Orc"][standingPOIClass][standingPOI] then tHSfaction = "H" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Troll"][standingPOIClass] then	if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Troll"][standingPOIClass][standingPOI] then tHSfaction = "H" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Tauren"][standingPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Tauren"][standingPOIClass][standingPOI] then tHSfaction = "H" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Undead"][standingPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Undead"][standingPOIClass][standingPOI] then tHSfaction = "H" end end	
		if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Goblin"][standingPOIClass] then if CensusPlus_Database.Servers["Turtle WoW"].TURTLE["Goblin"][standingPOIClass][standingPOI] then tHSfaction = "H" end 	end	
		
		if tHSfaction == HSfaction then
			standingnew = standingnew +1
		end
		
		--Debug shit
		--if standing == -1 then
		--	print(standingnew.." ("..i.."): "..t[i][1].." "..t[i][2].." "..tHSfaction.." v "..HSfaction)
		--end
	...
	if (playerOfInterest ~= playerName) then
		DEFAULT_CHAT_FRAME:AddMessage("- HonorSpy v"..tostring(VERSION)..": "..L["Report for player"].." "..playerOfInterest)
	end
	DEFAULT_CHAT_FRAME:AddMessage("- HonorSpy v"..tostring(VERSION)..": "..L["Pool Size"].." = "..pool_size..", "..L["Standing"].." = "..standing.." ("..standingold.."),  "..L["Bracket"].." = "..my_bracket..",  "..L["current RP"].." = "..RP..",  "..L["Next Week RP"].." = "..EstRP)
	DEFAULT_CHAT_FRAME:AddMessage("- HonorSpy v"..tostring(VERSION)..": "..L["Current Rank"].." = "..Rank.." ("..Progress.."%), "..L["Next Week Rank"].." = "..EstRank.." ("..EstProgress.."%)")
end
...
function BuildMenu()
	...
	options.args["display"] = {
		type = "text",
		name = L["Display"],
		desc = L["Faction Display"],
		get = function() return HonorSpy.db.realm.hs.display end,
		set = function(v)
			HonorSpy.db.realm.hs.display = v;
			HonorSpyStandings:Refresh();
		end,
		validate = {L["Alliance"], L["Horde"], L["Cross-Faction"]},
	}
	options.args["export"] = {
		type = "execute",
